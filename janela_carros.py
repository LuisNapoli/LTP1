...
class Janela_carros(Toplevel):
    def __init__(self, parent, control):
        ...
        self.title('Carros a venda')
        self.geometry('400x400+100+100')
        self.transient(parent)
        self.grab_set()
        Label(self, text='Dados do Carro').grid(row=0, column=2, padx=20, pady=2)
        Label(self, text='').grid(row=0, column=0, padx=20, pady=2)
        Label(self, text='Remover Carro').grid(row=43, column=2, padx=20, pady=2)

        self.btn_add = Button(self, text='Adicionar carro', command=self.adc_car).\
            grid(row=8, column=1, columnspan=3, pady=10, stick=S)
        self.bt_rmv = Button(self, text='Remover carro',fg='red', command=self.remove_car).\
            grid(row=52, column=1, columnspan=3, pady=10, stick=S)

        self.btn_close = Button(self, text='Atualizar', command=self.destroy, width=10)
        self.btn_close.grid(row=55, column=1, columnspan=3, stick=S, pady=20)
        ...
        self.entry_placa = Entry(self, textvariable=self.entry_placa_var).\
            grid(row=6, column=3)
        self.lbl_placa = Label(self, text='Chassi').\
            grid(row=6, column=1, stick=E)

        self.entry_modelo_var2 = StringVar()
        self.entry_modelo2 = Entry(self, textvariable=self.entry_modelo_var2). \
            grid(row=48, column=3)
        self.lbl_modelo2 = Label(self, text='Modelo'). \
        ...
        self.entry_placa2 = Entry(self, textvariable=self.entry_placa_var2). \
            grid(row=49, column=3)
        self.lbl_modelo2 = Label(self, text='Chassi'). \
            grid(row=49, column=1, stick=E)
    ...
    def remove_car(self):
        ...
        modelo = self.entry_modelo_var2.get()
        rmv = None
        if messagebox.askyesno('Excluir', f'Tem ceteza que deseja excluir o carro: {modelo}?') is False:
            return None
        for c in self.control.bd.show_carros():
            if c.get_modelo() == modelo:
                rmv = self.control.bd.remove_car(c)
            if c.get_placa() == placa:
                rmv = self.control.bd.remove_car(c)
                messagebox.showinfo('Carro', f'{modelo} foi removido.')
        if rmv is None:
